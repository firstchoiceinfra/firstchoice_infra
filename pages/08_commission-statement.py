...
st.markdown("""<style>
    .a4-page { background: white; padding: 50px; border: 3px solid #b8860b; color: black; max-width: 800px; margin: auto; }
    .comp-name { text-align: center; color: #b8860b; font-size: 50px; font-weight: 900; text-transform: uppercase; margin: 0; }
    .comp-slogan { text-align: center; color: #1e3a8a; font-size: 18px; font-style: italic; margin-bottom: 5px; }
    .comp-address { text-align: center; font-size: 14px; color: #555; margin-bottom: 30px; }
...
if 'df_view' in st.session_state and st.session_state.df_view is not None:
    ...
    st.markdown(f"""<div class='header-sect'>
        ...
        <p class='comp-address'>📍 Plot No. 06, Shop No.106, Motilal Nagar, Gonhi(Sim) Bahadura, Nagpur-440034</p>
    ...
    if st.button("🖨️ Print / Save as PDF"): st.markdown("<script>window.print();</script>", unsafe_allow_html=True)
    
    msg = f"FIRSTCHOICE INFRA - Commission Summary%0APartner: {meta['exec']}%0APayout: ₹{df['Net In Hand'].sum():,.2f}"
