...
def get_all_downlines(manager_name):
    ...
    for ex_name, details in exec_data_root.items():
        if str(details.get('senior_name', '')).strip().lower() == manager_clean:
            downlines.append(ex_name); downlines.extend(get_all_downlines(ex_name))
...
def prepare_edit(ex_name, details):
    st.session_state['form_exec_name'] = ex_name; st.session_state['form_senior_name'] = details.get('senior_name', '')
    st.session_state['form_exec_mobile'] = details.get('mobile', ''); st.session_state['ep'] = safe_float(details.get('percentage_exec', 0.0))
    st.session_state['er'] = safe_float(details.get('rupees_exec', 0.0)); st.session_state['edit_mode_active'] = True; st.session_state['old_edit_name'] = ex_name

def clear_edit_fields():
    for k in ['form_exec_name', 'form_senior_name', 'form_exec_mobile', 'ep', 'er', 'edit_mode_active', 'old_edit_name']: st.session_state.pop(k, None)
...
if exec_clean_list:
    ...
    comm_filter = st.radio("🎯 View Type:", ["All (Self + Team)", "⭐ Self Commission Only", "👥 Team Commission Only"], horizontal=True)
    ...
    if st.button("🔍 Generate Comprehensive Ledger", use_container_width=True):
        ...
        for p_name in project_names:
            ...
            p_mode = p_info.get('comm_type', 'Percentage (%)'); p_mauza = p_info.get('mauza', 'Unknown')
            for plot_id, plot_info in (p_info.get('plots', {}) if isinstance(p_info.get('plots', {}), dict) else {str(i):p for i,p in enumerate(p_info.get('plots', []))}).items():
                if isinstance(plot_info, dict) and str(plot_info.get('status', '')).lower() == 'booked':
                    ...
                    if (comm_filter == "⭐ Self Commission Only" and plot_exec != search_exec_clean) or (comm_filter == "👥 Team Commission Only" and plot_exec not in all_downlines_lower): continue
                    if plot_exec == search_exec_clean or plot_exec in all_downlines_lower:
                        sr_pct = safe_float(exec_data_root[search_exec].get('percentage_exec', 0.0))
                        sr_rs = safe_float(exec_data_root[search_exec].get('rupees_exec', 0.0))
                        
                        jr_pct, jr_rs, branch = get_diff_deduction(plot_info.get('executive_name', ''), search_exec_clean)
                        final_pct = sr_pct if plot_exec == search_exec_clean else max(0.0, sr_pct - jr_pct)
                        final_rs = sr_rs if plot_exec == search_exec_clean else max(0.0, sr_rs - jr_rs)
                        
                        for pmt in [{'date_str': str(plot_info.get('booking_date', plot_info.get('receipt_date', ''))), 'type': 'Booking Token', 'amt': safe_float(plot_info.get('token_amount', plot_info.get('received_amount', 0.0)))}] + [{'date_str': str(p.get('date', '')), 'type': str(p.get('remarks', 'Installment')), 'amt': safe_float(p.get('amount', 0.0))} for p in plot_info.get('partial_payments', [])]:
                            p_date = datetime.datetime.strptime(pmt['date_str'], "%Y-%m-%d").date() if pmt['date_str'] else datetime.date.today()
                            if start_date <= p_date <= end_date and pmt['amt'] > 0:
                                gross = (pmt['amt'] * final_pct / 100.0) if 'Percentage' in p_mode else (final_rs if pmt['type'] == 'Booking Token' else 0)
                                tds = (gross * 2.0) / 100.0
                                statement_rows.append({"S.No.": s_no, "Sale Origin": "⭐ Direct" if plot_exec == search_exec_clean else f"👥 Team ({branch})", "Client Name": str(plot_info.get('customer_name', 'N/A')).title(), "Project": f"{p_name} ({p_mauza})", "Plot": plot_id, "Payment": pmt['type'], "Paid": f"{pmt['amt']:,.0f}", "Date": p_date.strftime("%d-%m-%Y"), "Gross (₹)": round(gross, 2), "TDS (₹)": round(tds, 2), "Net Payout (₹)": int(round(gross - tds))})
                                s_no += 1
        ...
        if statement_rows:
            ...
            c1.metric("Total Gross Value", f"₹ {t_gross:,.2f}"); c2.metric("Total TDS Deduction", f"₹ {t_tds:,.2f}"); c3.metric("🏆 Grand Net Payable", f"₹ {t_net:,.2f}")
...
# Registry Grid
if user_role == 'admin':
    for ex_name, p_details in {k: v for k, v in exec_data_root.items() if isinstance(v, dict) and 'name' in v}.items():
        st.markdown(f'<div class="ledger-box"><b>Name:</b> {ex_name} | <b>Senior:</b> {p_details.get("senior_name")} | <b>Slab:</b> {p_details.get("percentage_exec")}%</div>', unsafe_allow_html=True)
